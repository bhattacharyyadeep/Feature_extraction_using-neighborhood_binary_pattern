...
for m=(((n-1)*25)+1):(25*n)
    ...
    [lbp_pattern,feature_query]=LNDP(a);%using LNDP function extracting query image features.
dist=zeros(size(2800));
filename='LNDP_feature.txt';% all images feature vector stored in LNDP_feature.txt
...
display("Precision of LNDP = "+precision_db*100); % ultimately displaying the over all ARP of the database.
